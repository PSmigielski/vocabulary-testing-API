...
Word.create = (newWord, result) => {
  db.query(
    `SELECT * FROM \`words\` WHERE \`word\` = '${newWord.word}'`,
    (err, res) => {
      if (err) {
        result(err, null);
        return;
      ...
      if (res.length > 0) {
        ...
        db.query(
          ...
          [newWord.id, newWord.word, newWord.definition, newWord.section_id],
          (err, res) => {
            if (err) {
              result(err, null);
              return;
    ...
    .on("file", async (name, file) => {
      ...
      const loadData = (file) => {
        return new Promise((res, rej) => {
          try {
            db.query(
              `LOAD DATA LOCAL INFILE '${__dirname}/${file.name}' INTO TABLE \`words\` FIELDS TERMINATED BY ";" LINES TERMINATED BY "\n" `,
              (err, data) => {
                if (err) {
                  throw err;
                } else res(data);
...
Word.show = (sectionID, result) => {
  db.query(
    `SELECT * FROM \`words\` WHERE \`section_id\` = ${sectionID}`,
    (err, data) => {
      if (err) {
        result(err, null);
        return;
